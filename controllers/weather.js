...
// Global functions for manipulating data
 ...
 // },
  ...
  putNewCity: async (req, res) => {
    ...
    // Loop through the userCityList array and fetch weather information for each city
      ...
      try {
        // Make API request to fetch weather information for the city
        const weatherResponse = await fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&appid=${process.env.OW_API}&units=imperial&exclude=minutely`);
        owData = await weatherResponse.json();
      
        // Make API request to fetch air quality information for the city
        const aqResponse = await fetch(`http://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${process.env.OW_API}`);
        aqData = await aqResponse.json();
